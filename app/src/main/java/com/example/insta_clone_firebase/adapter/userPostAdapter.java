...
public class userPostAdapter extends RecyclerView.Adapter<userPostAdapter.ViewHolder>{
    ...
    private ArrayList<post_create> postList;
    Context context;
    public userPostAdapter(ArrayList<post_create> postList, Context context) {
...
// post comments
    ...
    private void removePostFromDB(int pos) {
        post_create post = postList.get(pos);

        StorageReference ref = FirebaseStorage.getInstance().getReferenceFromUrl(post.getPost_url());
        System.out.println("Post url ::" + post.getPost_url());
        ref.delete().addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void unused) {
                FirebaseFirestore db = FirebaseFirestore.getInstance();
                DocumentReference ref1 = db.collection("Instagram User Private Data").document(HomeScreenActivity.USER_DATA.getUser_id());
                ref1.update("posts",FieldValue.arrayRemove(post.getPost_uid())).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void unused) {

                        DocumentReference ref2 = db.collection("Instagram_user_post_data").document(post.getPost_uid());
                        ref2.delete().addOnSuccessListener(new OnSuccessListener<Void>() {
                            @Override
                            public void onSuccess(Void unused) {
                                System.out.println("Posts removed");
                                postList.remove(pos);
                            ...
                            public void onFailure(@NonNull Exception e) {
                                System.out.println("Error in removing post from post db :: " + e.getMessage());
                                e.printStackTrace();
                            }
                        });
                    }
                    ...
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        System.out.println("Error in removing posts from db" + e.getMessage());
                        e.printStackTrace();
                    }
                });
            }
            ...
            @Override
            public void onFailure(@NonNull Exception e) {
                System.out.println("Error in removing posts from storage" + e.getMessage());
                e.printStackTrace();
            }
        });
    }

    private void deletePostOfUser(post_create post) {
        ...
        ref.delete().addOnSuccessListener(new OnSuccessListener<Void>() {
            ...
            public void onSuccess(Void unused) {
                FirebaseFirestore db = FirebaseFirestore.getInstance();
                DocumentReference ref1 = db.collection("Instagram User Private Data").document(HomeScreenActivity.USER_DATA.getUser_id());
                ref1.update("posts",FieldValue.arrayRemove(post)).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void unused) {
                        System.out.println("Posts removed");
                        get_user_db.loadUserData(context);
